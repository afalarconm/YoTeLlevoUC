...
export default function App() {
    ...
    const deleteViaje = (id) => {
        Axios.delete(`http://localhost:3001/viajes/${id}`).then(() => {
    ...
    const Tabla=post.map(
        (info)=>{
            ...
            return(
                ...
                <button onClick={() => editViaje(info.id)} className="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" style={{display: token ? "": "none"}}>Editar</button>
                <button onClick={() => deleteViaje(info.id)} className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" style={{display: token ? "": "none"}}>Eliminar</button>
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-4">
            <div class="sm:flex sm:items-center">
                ...
                <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                    <button type="button" onClick={() => navigate("/CreateViaje")}  style={{display: token ? "": "none"}} class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">Añadir viaje</button>
                </div>
            </div>
